...
DataTree = function (params, subTreeRequest) {
    ...
    function actOnXML(xml, isSubTreeRequest) {
        ...
        if (params.openAtPath && !isSubTreeRequest) {
            let xpRes = this.xml.evaluate(params.openAtPath, this.xml, null, XPathResult.UNORDERED_NODE_SNAPSHOT_TYPE),
                itNode;
    ...
    function postBuild(isSubTreeRequest) {
        ...
        if (params.clickCallback)
            this.tree.on('.LIText', 'click', function (evt) {
                ...
                params.clickCallback(li, returnXPathToNode(li), evt);
            });
    ...
    if (params.fpath) {
        ...
        $.ajax({
            ...
            .error(function () {
                debug('could not load XML from ' + params.fpath);
            })
        ...
        if (!params.noTagRenaming)
            this.xml = params.xml
                .replace(/<(\/)?(\w+)([^>]*)>/g, (...rpl) => {
                    return '<' + (rpl[1] ? rpl[1] : '') + rpl[2] + '_' + rand + (rpl[3] ? rpl[3] : '') + '>';
                })
                .replace(/<\?xml[^>]+>\s*/, '');
    ...
    function returnXPathToNode(nodeEl) {
        ...
        nodeEl.parents('li').andSelf().each(function () {
            ...
            let step = nodeName;
            let index = $(this).prevAll().filter(function () {
                return $(this).children('.LIText').children('.node').text() == nodeName;
            }).length + 1;
